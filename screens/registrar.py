...
class MedidasManagerDialog(QtWidgets.QDialog):
    ...
    def _build_ui(self):
        ...
        self.btn_del = QtWidgets.QPushButton("Desactivar Seleccionada")
        ...
        self.btn_use = QtWidgets.QPushButton("✅ Usar Medida")
...
class RegistrarForm(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.is_saving = False  # SEMÁFORO DE SEGURIDAD
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        # --- VALIDACIÓN DE LOTE (1-3 DÍGITOS) ---
        self.nro_lote = self._create_input("Ej: 101")
        rx = QtCore.QRegularExpression("^[0-9]{1,3}$")
        self.nro_lote.setValidator(QtGui.QRegularExpressionValidator(rx, self.nro_lote))

        # --- CAMBIO: FECHA CON PLACEHOLDER ---
        ...
        self.rows = {}
        self.rows['nro_lote'] = self._add_row(form_layout, "Nro. Lote (1-3 díg):", self.nro_lote)
    ...
    def _create_input(self, ph):
        le = QtWidgets.QLineEdit()
        le.setPlaceholderText(ph)
        le.setStyleSheet(f"background-color: {theme.BG_INPUT}; padding: 5px; border: 1px solid {theme.BORDER_COLOR}; color: white; border-radius: 4px;")
        return le
    ...
    # --- CAMBIO: CONFIGURACIÓN DEL "PLACEHOLDER" DE FECHA ---
    def _create_date_edit(self):
        ...
        de.setSpecialValueText("dd/mm/aaaa")
        
        # 3. Iniciar con la fecha mínima (mostrando el texto)
        de.setDate(min_date)
    ...
    def _validate_input(self, tipo):
        ...
        if not self.nro_lote.text().strip():
            QtWidgets.QMessageBox.warning(self, "Falta Lote", "Debe ingresar un **Número de Lote** (1-3 dígitos).")
            self.nro_lote.setFocus()
            return False

        # Validar si el usuario NO ha cambiado la fecha inicial
        if self.prod_date.date() == self.prod_date.minimumDate():
            QtWidgets.QMessageBox.warning(self, "Falta Fecha", "Debe seleccionar la **Fecha de Producción**.")
            self.prod_date.setFocus()
            return False
    ...
    def _on_save(self):
        ...
        self.save_btn.setText("Procesando...")
        
        try:
            tipo = self.product_type.currentText()
            if tipo.startswith("--"): raise ValueError("Seleccione un producto.")
            
            # Llamada a la función validadora (que ahora sí existe y está indentada)
            ...
            # Mensaje detallado
            msg = (f"¿Confirmar registro?\n\n📦 Producto: {tipo}\n🏷️ Lote: {self.nro_lote.text()}\n"
                   f"📅 Fecha: {self.prod_date.date().toString('dd/MM/yyyy')}\n"
                   f"📊 Cantidad: {cant_bultos} Bultos\n(Total: {total_piezas:.0f} piezas)")

            if QtWidgets.QMessageBox.question(self, "Confirmar", msg, 
            ...
            data = {
                ...
                "obs": self.obs.toPlainText() + f" (Entrada: {cant_bultos} Bultos)"
            }
            ...
            if result.get("status") == "ignored_duplicate":
                # Es un duplicado exacto (doble clic), mostramos éxito sin duplicar
            ...
            # Solo mostramos alerta si es un error real
            if str(e) != "Datos inválidos.": 
                QtWidgets.QMessageBox.warning(self, "Atención", str(e))
    ...
    def _clear_form(self):
        ...
        self.obs.clear()
        # RESETEAR FECHA A MÍNIMA (para que salga el texto "dd/mm/aaaa")
        self.prod_date.setDate(self.prod_date.minimumDate())
